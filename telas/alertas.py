...
# Substitua o token abaixo pela sua chave secreta (re_...) copiada do Resend
...
def verificar_e_enviar_alertas(db, empresa_id: int, email_destino: str):
    ...
    try:
        # Busca todos os produtos sem filtros para garantir que lemos a sua lista
        ...
        for p in produtos:
            ...
            # Força a entrada do produto de teste ou faz a validação real
        ...
        # Garante que temos algo para enviar
    ...
    if btn_verificar:
        with st.spinner("Varrendo o Supabase e estruturando relatório em HTML..."):
            ...
            if enviou:
                ...
                st.info("🔍 **Tudo em ordem por aqui!** O sistema rodou a varredura, mas nenhuma inconsistência foi detectada.")
